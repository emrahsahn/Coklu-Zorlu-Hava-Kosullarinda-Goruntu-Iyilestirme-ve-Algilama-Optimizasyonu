...
class RESIDE_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = self.haze_imgs[index]
        filename = os.path.basename(img)  # örn: 001.png
        clear = Image.open(os.path.join(self.clear_dir, filename))
...
ITS_train_loader = DataLoader(
    dataset=RESIDE_Dataset(path+'/train', train=True, size=crop_size),
    batch_size=BS,
    shuffle=True
)

ITS_test_loader = DataLoader(
    dataset=RESIDE_Dataset(path+'/test', train=False, size='whole img'),
    batch_size=1,
    shuffle=False
)
