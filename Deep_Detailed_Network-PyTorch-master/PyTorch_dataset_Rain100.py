...
class Rain100(data.Dataset):
    ...
    def _scan(self):
        ...
        list_n = sorted(
            [os.path.join(self.dir_n, x)
            for x in os.listdir(self.dir_n)
            if is_img(x)]
        )
    ...
    def _load_file(self, idx):
        ...
        if self.args.ext == 'img':
            img_n = imageio.imread(file_n)
            if np.max(img_n) > 1:
                img_n = img_n / 255.0

            img_c = imageio.imread(file_c)
            if np.max(img_c) > 1:
                img_c = img_c / 255.0
...
class Rain100_Test(data.Dataset):
    def __init__(self, args):
        ...
        self._set_filesystem(args.input_path_test, args.input_path_test)
